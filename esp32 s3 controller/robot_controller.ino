/*
 * ESP32S3 — Robot Controller
 * Motores + Telemetría Continua en Grados + Color (AS7341) + Distancia (VL53L1X)
 *
 * I2C unificado → SDA: GPIO 8 | SCL: GPIO 9
 * (evita los problemáticos GPIO 10/11 del flash en módulos genéricos)
 */
...
// ── I2C UNIFICADO: GPIO 8 (SDA) y GPIO 9 (SCL) ──────────────
// Ambos sensores (AS7341 + VL53L1X) comparten este bus.
// AS7341 → dirección 0x39
// VL53L1X → dirección 0x29
// No hay conflicto de direcciones.
#define I2C_SDA 8
#define I2C_SCL 9
...
volatile int  sDist    = -1;   // mm, -1 = no data
...
float evalExpr(const char* expr) {
  ...
  // Variable especial: distancia del sensor en mm
...
void setup() {
  ...
  // I2C unificado en GPIO 8/9 para ambos sensores
  Wire.begin(I2C_SDA, I2C_SCL);

  // AS7341 — sensor de color
  if (as7341.begin()) {
  ...
  // VL53L1X — sensor de distancia
  if (distanceSensor.begin() == 0) {
    distanceSensor.setDistanceModeLong();   // hasta ~4 m
...
void loop() {
  ...
  // 3. Telemetría de Encoders (cada 200 ms)
