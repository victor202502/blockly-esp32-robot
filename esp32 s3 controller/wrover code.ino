/*
 * ESP32 WROVER — Robot Controller (ESTABLE - SIN PID)
 * Motores independientes + Telemetría + Sensores I2C
 */
...
// ── PINES MOTORES Y ENCODERS (ADAPTADOS A ESP32-WROVER) ──────
#define PWMA 25
#define AIN1 26
#define AIN2 27
...
bool sensorDistOk = false;

volatile int  sDist    = -1;   
volatile bool sDistNew = false;

void distTaskFn(void* arg) {
  for (;;) {
    if (sensorDistOk) {
...
void IRAM_ATTR isrA() {
  if (digitalRead(ENC_A_B) == HIGH) encA++; else encA--;
}
void IRAM_ATTR isrB() {
  if (digitalRead(ENC_B_B) == HIGH) encB++; else encB--;
}
...
float evalExpr(const char* expr) {
  ...
  if (strcmp(ex, "DIST") == 0) return (float)sDist;
...
// ── MOTORES (CONTROL SIMPLIFICADO Y ESTABLE) ─────────────────
...
// Lógica de pines ajustada al driver TB6612
void dirA(bool f) { digitalWrite(AIN1, f?HIGH:LOW); digitalWrite(AIN2, f?LOW:HIGH); }
...
void moveSteps(long steps, bool fwd, int vel) {
  ...
  motorOn(); 
  dirA(fwd); 
  dirB(!fwd); // Invertimos la lógica B porque el motor B está montado del revés
  ...
  while(!motAbort) {
    ...
    if(dA >= steps && dB >= steps) break; // Ambos terminaron
    
    // Parada individual: Si uno llega primero, lo apagamos, el otro sigue.
    if(dA < steps) setPWM_A(pwm); else setPWM_A(0);
    if(dB < steps) setPWM_B(pwm); else setPWM_B(0);
...
void turnSteps(long steps, bool cw, int vel) {
  ...
  // Para rotar, ambos motores giran en su propia dirección "física" forward/backward igual.
  dirA(!cw); 
  dirB(!cw);
...
void moveSingleMotor(int motor, long steps, bool fwd, int vel) {
  int pwm = scalePWM(vel);
  motorOn();
  
  if (motor == 1) { 
    encA = 0; dirA(fwd); setPWM_A(pwm); setPWM_B(0); 
  } else { 
    encB = 0; dirB(!fwd); setPWM_B(pwm); setPWM_A(0); 
  }
  
  sendTelem("STATUS", motor == 1 ? "MOTOR_A" : "MOTOR_B");
  
  while(!motAbort) {
    if (motor == 1 && abs(encA) >= steps) break;
    if (motor == 2 && abs(encB) >= steps) break;
    vTaskDelay(pdMS_TO_TICKS(5));
  }
  motorOff();
}
...
void tankMove(float vA, float vB, float secs) {
  ...
  while(!motAbort&&(millis()-t0)<(unsigned long)(secs*1000.0f)) {
    vTaskDelay(pdMS_TO_TICKS(5));
  }
  motorOff();
}
...
void execLine(int& pc) {
  ...
  else if (t=="MOTOR_A")    { float r=evalExpr(getP(line,1).c_str()); bool fwd=(getP(line,2)!="0"); int sp=(int)evalExpr(getP(line,3).c_str()); moveSingleMotor(1, (long)(r*PASOS_X_VUELTA),fwd,sp); }
  else if (t=="MOTOR_B")    { float r=evalExpr(getP(line,1).c_str()); bool fwd=(getP(line,2)!="0"); int sp=(int)evalExpr(getP(line,3).c_str()); moveSingleMotor(2, (long)(r*PASOS_X_VUELTA),fwd,sp); }
...
void setup() {
  ...
  if (as7341.begin()) {
    ...
    Serial.println("AS7341 no detectado");
  }
  ...
  if (distanceSensor.begin() == 0) {
    ...
    xTaskCreatePinnedToCore(distTaskFn, "DistTask", 4096, NULL, 1, NULL, 0);
  } else {
    Serial.println("VL53L1X no detectado");
  }
  ...
  attachInterrupt(digitalPinToInterrupt(ENC_A_A),isrA,RISING);
  attachInterrupt(digitalPinToInterrupt(ENC_B_A),isrB,RISING);
...
void loop() {
  ...
  if (sDistNew && wemosKnown) {
    sDistNew = false;
    char buf[16];
    snprintf(buf, sizeof(buf), "%d", sDist);
    sendTelem("DIST", buf);
  }
